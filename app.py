...
# Full Rewrite v3.0 — Professional SaaS Edition (Bug-Free)
# =============================================================================
...
AI_AGENTS = [
    {"id":1,  "name":"Visionary",   "role":"Strategic AI Architect",       "icon":"🧠",
     "desc":"Memetakan strategi keamanan bisnis & skenario ROI jangka panjang."},
    {"id":2,  "name":"Concierge",   "role":"Customer Intelligence AI",     "icon":"🤝",
     "desc":"Mendeteksi kebutuhan klien & merekomendasikan paket secara proaktif."},
    {"id":3,  "name":"Sentinel CS", "role":"24/7 AI Sales Consultant",     "icon":"🛡️",
     "desc":"Chatbot penjualan utama — aktif di seluruh halaman."},
    {"id":4,  "name":"Liaison",     "role":"Fraud Intelligence Engine",    "icon":"🔍",
     "desc":"Filter lokal anomali/void/fraud sebelum dikirim ke API — hemat 80% biaya."},
    {"id":5,  "name":"Auditor",     "role":"Bank Statement AI",            "icon":"🏦",
     "desc":"Rekonsiliasi otomatis laporan kasir vs mutasi rekening bank."},
    {"id":6,  "name":"Stockmaster", "role":"Inventory Intelligence AI",    "icon":"📦",
     "desc":"Baca invoice supplier via OCR & update stok real-time tanpa input manual."},
    {"id":7,  "name":"VisualEye",   "role":"CCTV Overlay AI",              "icon":"📹",
     "desc":"Render teks transaksi langsung di atas feed CCTV real-time."},
    {"id":8,  "name":"WhisperBot",  "role":"WhatsApp Notification AI",     "icon":"📲",
     "desc":"Kirim alarm fraud & laporan harian ke Owner via WhatsApp otomatis."},
    {"id":9,  "name":"Collector",   "role":"A/R Collection Reminder AI",   "icon":"💰",
     "desc":"Ingatkan piutang jatuh tempo H-7 & H-1 secara otomatis."},
    {"id":10, "name":"Oracle",      "role":"Predictive Analytics AI",      "icon":"🔮",
     "desc":"Proyeksi omzet, deteksi tren kebocoran, dan rekomendasi tindakan preventif."},
]
...
def fetch_pos_data(api_url="http://localhost:8080/api/transactions", api_key=""):
    """
    Fetch real-time POS transaction data from local kasir server.
    TODO (production): replace with actual requests.get() call.
    """
...
def process_yolo_cctv_frame(frame_data):
    ...
    TODO (production): integrate with ultralytics YOLO model.
    """
    return {
...
def trigger_alarm(reason, kasir, cabang, amount, wa_owner=WA_NUMBER):
    """
    Generate a WhatsApp fraud alarm link to the business owner.
    """
    msg = (
        "🚨 *ALERT V-GUARD AI*\n\n"
        "📍 Cabang: " + cabang + "\n"
...
/* ================================================================
   CONCIERGE CHAT WIDGET — Fixed, high z-index, fully responsive
   ================================================================ */
#vg-fab-wrap {
...
#vg-chatbox {
    ...
    box-shadow: 0 12px 48px #00000099 !important;
...
.vg-typing span {
    ...
    animation: vgDot 1.3s infinite ease;
}
...
.feature-card:hover {
    ...
    box-shadow: 0 8px 30px #00d4ff11;
}
.feature-title {
    font-family: 'Rajdhani', sans-serif !important;
...
/* Package Cards */
.pkg-card {
    background: #101c2e; border: 1px solid #1e3352; border-radius: 14px;
...
.pkg-card-ultra {
    background: linear-gradient(160deg,#12100a,#1a1500,#0e0e0e);
    border: 1px solid #ffd70055; border-radius: 14px; padding: 22px 16px 20px;
    display: flex; flex-direction: column; height: 100%; transition: all .3s ease; position: relative;
}
.pkg-card-ultra:hover { border-color:#ffd700; box-shadow:0 0 32px #ffd70022; transform:translateY(-4px); }
.pkg-card-popular {
    background: #101c2e; border: 1.5px solid #0091ff; border-radius: 14px;
    padding: 22px 16px 20px; display: flex; flex-direction: column;
    height: 100%; transition: all .3s ease; position: relative;
}
...
.hot-label {
    ...
    background:linear-gradient(135deg,#0091ff,#00d4ff); color:#000 !important;
    font-family:'Rajdhani',sans-serif !important; font-size:10px !important; font-weight:700 !important;
    padding:3px 14px; border-radius:20px; letter-spacing:1px; white-space:nowrap;
}
.ultra-label {
    position:absolute; top:-12px; left:50%; transform:translateX(-50%);
    background:linear-gradient(135deg,#b8860b,#ffd700); color:#000 !important;
    font-family:'Rajdhani',sans-serif !important; font-size:10px !important; font-weight:700 !important;
    padding:3px 14px; border-radius:20px; letter-spacing:1px; white-space:nowrap;
}
.tier-badge {
    display:inline-block; font-family:'JetBrains Mono',monospace !important;
...
# =============================================================================
# 12. THE CONCIERGE CHAT WIDGET — Bug-free, fully responsive
# =============================================================================
CHAT_WIDGET_HTML = r"""
<div id="vg-fab-wrap">
        ...
        <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:10px;color:#00e676;font-family:'JetBrains Mono',monospace;">
                <span style="display:inline-block;width:6px;height:6px;border-radius:50%;
                             ...
                             animation:vgPulse 2s infinite;"></span>Online · Siap membantu
            </div>
        </div>
        <button id="vg-close" aria-label="Tutup chat"
                style="background:transparent;border:none;color:#7a9bbf;font-size:22px;
                       cursor:pointer;padding:4px 8px;line-height:1;border-radius:6px;
                       transition:color .15s;outline:none;flex-shrink:0;">✕</button>
    </div>
...
(function() {
    ...
    if (!fab || !chatbox || !msgs || !input || !sendBtn || !closeBtn) {
        console.warn('V-Guard Chat: DOM elements not found');
        return;
    }
    ...
    function showTyping() {
        var existing = document.getElementById('vg-typing');
    ...
    var rules = [
        ...
        },
        {
            ...
            reply: 'Demo V-Guard <strong>GRATIS 30 menit</strong> — Pak Erwin langsung tunjukkan cara sistem mendeteksi kecurangan secara real-time!<br><br>📲 <a href="https://wa.me/6282122190885?text=Halo+Pak+Erwin%2C+saya+ingin+Book+Demo+V-Guard+AI." target="_blank" style="color:#00d4ff;">Klik di sini untuk Book Demo</a>'
        },
        {
        ...
        },
        {
            ...
            reply: 'Program Referral V-Guard memberikan <strong>komisi 10%</strong> dari biaya setup klien baru yang Anda referensikan! 💰<br><br>Daftarkan bisnis Anda di Portal Klien, lalu dapatkan <strong>link referral unik</strong> Anda. Bagikan ke rekan bisnis &amp; komisi masuk otomatis!<br><br>Mau saya bantu daftarkan bisnis Anda sekarang?'
        },
        {
            keys:  ['daftar','order','beli','aktivasi','mulai','gabung'],
            reply: 'Siap! Untuk memulai, Bapak/Ibu bisa pilih paket yang sesuai:<br><br>🔵 V-LITE (Rp 150rb) · ⚡ V-PRO (Rp 450rb) · 🟣 V-ADVANCE (Rp 1,2jt)<br>🟢 V-ELITE (Rp 3,5jt) · 👑 V-ULTRA (Custom)<br><br>Boleh saya tahu jenis usaha Bapak/Ibu? 😊'
        },
    ];

    function roiCalc(msg) {
        var m = msg.toLowerCase();
        if (!(m.indexOf('omzet')!==-1 || m.indexOf('roi')!==-1 || m.indexOf('juta')!==-1 || m.indexOf('jt')!==-1)) return null;
        var re = /(\d[\d.,]*)\s*(juta|jt|miliar|m\b|rb|ribu)?/i;
        var match = m.match(re);
        if (!match) return null;
        var raw  = parseFloat(match[1].replace(/,/g,'.').replace(/\./g,'')) || 0;
        var unit = (match[2]||'').toLowerCase();
        var omzet = raw;
        if (unit==='juta'||unit==='jt')    omzet = raw * 1e6;
        else if (unit==='miliar'||unit==='m') omzet = raw * 1e9;
        else if (unit==='rb'||unit==='ribu')  omzet = raw * 1e3;
        if (omzet < 1e6) return null;
        ...
        return (
            '💡 <strong>Estimasi ROI V-Guard:</strong><br><br>' +
            '📊 Omzet: Rp ' + fmt(omzet) + '/bln<br>' +
            '💸 Kebocoran ~5%: Rp ' + fmt(bocor) + '/bln<br>' +
            '✅ Diselamatkan (88%): <strong>Rp ' + fmt(saved) + '/bln</strong><br>' +
    ...
    function getFallback(msg) {
        ...
        return (
            'Halo! 👋 Saya <strong>Sentinel CS</strong>, konsultan AI V-Guard.<br><br>' +
            'Ceritakan bisnis Bapak/Ibu:<br>' +
            '• Berapa <strong>kasir/cabang</strong>?<br>' +
    ...
    function processMsg(text) {
        ...
        isBusy = true;
        var clean = text.trim();
        addBubble(escHtml(clean), true);
        showTyping();
        var delay = 700 + Math.random() * 800;
        setTimeout(function() {
            removeTyping();
            var reply = getFallback(clean);
    ...
    pills.forEach(function(pill) {
        pill.addEventListener('click', function(e) {
            ...
            if (msg) {
                if (!isOpen) openChat();
                setTimeout(function() { processMsg(msg); }, 100);
            }
        });
    });

    // Prevent chatbox click from bubbling to document
    ...
    document.addEventListener('click', function(e) {
        if (isOpen && !chatbox.contains(e.target) && e.target !== fab) {
            closeChat();
        }
    });
    ...
    setTimeout(function() {
        if (!isOpen && notif) notif.style.display = 'block';
    }, 4000);

    // Auto-open after 14s if not opened
    setTimeout(function() {
        if (!isOpen) openChat();
    }, 14000);

    console.log('V-Guard Sentinel CS Widget loaded successfully.');
})();
</script>
"""
...
with st.sidebar:
    ...
    <div style='padding:20px 0 16px;border-bottom:1px solid #1e3352;margin-bottom:16px;'>
        ...
        <div style='text-align:center;margin-top:8px;font-size:11px;color:#7a9bbf;
                    font-family:JetBrains Mono,monospace;'>
    ...
    # Public menu always visible; private only when authenticated
    ...
    # Admin / Investor login block (only shown when not authenticated)
...
if menu == "Beranda":
    ...
    for col, (text, author, role, result) in zip([t1, t2, t3], TESTI):
        with col:
            st.markdown(
                ...
                "font-size:12px;color:#00d4ff;font-family:JetBrains Mono,monospace;'>✓ " + result + "</div>"
                "</div>",
                unsafe_allow_html=True,
            )
    ...
    <div style='background:linear-gradient(135deg,#0d1a2e,#0a1628);padding:48px;
        ...
        <div style='font-family:Rajdhani,sans-serif;font-size:36px;font-weight:700;
                    color:#e8f4ff;margin-bottom:12px;'>Siap Menutup Kebocoran Bisnis Anda?</div>
        <div style='font-size:15px;color:#7a9bbf;margin-bottom:28px;'>
            Konsultasi gratis 30 menit dengan tim V-Guard AI. Tidak ada kewajiban apapun.</div>
    </div>""", unsafe_allow_html=True)
...
# =============================================================================
    ...
    PACKAGES = [
        ...
        },
        {
            ...
            "features": ["Semua fitur V-PRO","CCTV AI — Visual Cashier Audit",
                         "WhatsApp Fraud Alarm Instan","H-7 Auto Collection Reminder","Multi-Cabang Dashboard"],
        },
        {
            ...
            "popular": False, "ultra": False, "plug_play": False,
            "features": ["Semua fitur V-ADVANCE","Deep Learning Forensik AI","Dedicated Private Server",
        ...
        },
        {
            ...
            "features": ["Seluruh ekosistem V-ELITE","White-Label Platform","Executive BI Dashboard C-Level",
                         "10 Elite AI Squad serentak","Dedicated AI Strategist","V-Guard ERP Liaison Protocol"],
        },
    ]
    ...
    for pkg in PACKAGES:
        ...
        order_btn = (
            ...
            "border-radius:6px;text-decoration:none;letter-spacing:.5px;'>"
            "Order Sekarang</a>"
        )
        cards_html += (
    ...
    st.markdown(
        ...
        "width:100%;margin-bottom:24px;'>" + cards_html + "</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown(
        ...
        "<b style='color:#00e676;'>Plug &amp; Play (V-LITE &amp; V-PRO):</b> Setup mandiri, tanpa teknisi, langsung aktif dalam hitungan menit. "
        "&nbsp;&nbsp;<b style='color:#ffab00;'>Teknisi Profesional (V-ADVANCE ke atas):</b> Integrasi khusus oleh teknisi V-Guard di lokasi bisnis Anda."
        "</div></div>",
        unsafe_allow_html=True,
    )
...
# =============================================================================
    ...
    with col_form:
        st.markdown(
            "<div style='background:#101c2e;border:1px solid #1e3352;border-radius:12px;padding:24px;'>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
            "color:#00d4ff;margin-bottom:16px;'>Input Data Bisnis</div>",
            unsafe_allow_html=True,
        )
        jenis_usaha   = st.selectbox(
            "Jenis Usaha",
            ["Warung / 1 Kasir","Toko Ritel / Cafe","Minimarket","Multi-Cabang","Korporasi"],
            key="roi_jenis",
        )
        ...
        if jenis_usaha == "Warung / 1 Kasir":
            auto_pkg = "V-LITE"
        elif jenis_usaha in ("Toko Ritel / Cafe","Minimarket"):
            auto_pkg = "V-PRO"
        elif jenis_usaha == "Multi-Cabang":
            auto_pkg = "V-ADVANCE"
        else:
            auto_pkg = "V-ELITE"

        pkg_options   = ["V-LITE","V-PRO","V-ADVANCE","V-ELITE"]
        paket_dipilih = st.selectbox(
            "Paket Yang Ingin Dievaluasi",
            pkg_options,
            index=pkg_options.index(auto_pkg) if auto_pkg in pkg_options else 1,
            key="roi_paket",
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_result:
        ...
        st.markdown(
            "<div style='background:linear-gradient(135deg,#0d1626,#101c2e);border:1px solid #1e3352;"
            "border-radius:12px;padding:24px;margin-bottom:16px;'>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;color:#00d4ff;"
            "margin-bottom:20px;'>Hasil Kalkulasi — " + paket_dipilih + "</div>"
            ...
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;color:#ff3b5c;'>"
            "Rp {:,.0f}</div></div>".format(bocor_rp) +

            "<div style='background:#060b14;border:1px solid #1e3352;border-radius:8px;padding:16px;text-align:center;'>"
            "<div style='font-size:11px;color:#7a9bbf;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;'>Diselamatkan (88%)</div>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;color:#00e676;'>"
            "Rp {:,.0f}</div></div>".format(diselamatkan) +
            ...
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;color:#ffab00;'>"
            + hb + "</div></div>" +
            ...
            "Rp {:,.0f}</div></div>".format(net_saved) +

            "</div>"
            "<div style='background:linear-gradient(135deg,#00e67611,#00d4ff11);border:1px solid #00e67633;"
            ...
            "background:linear-gradient(135deg,#00e676,#00d4ff);-webkit-background-clip:text;"
            "-webkit-text-fill-color:transparent;'>{:.0f}%</div>".format(roi_pct) +
            "<div style='font-size:12px;color:#7a9bbf;'>Payback Period: ±{:.0f} hari</div>"
            "</div></div>".format(payback_hari),
            unsafe_allow_html=True,
        )
        ord_link = ORDER_LINKS.get(paket_dipilih, WA_LINK_KONSUL)
        st.link_button("Order " + paket_dipilih + " — " + hb + "/bln", ord_link, use_container_width=True)
...
# =============================================================================
    ...
    if st.session_state.client_logged_in and st.session_state.client_data:
        ...
        st.markdown(
            ...
            "border-radius:14px;padding:24px;margin-bottom:24px;display:flex;"
            "justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;'>"
            "<div>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:24px;font-weight:700;color:#e8f4ff;'>"
            "Selamat datang, " + klien.get("Nama Klien","Klien") + " 👋</div>"
            "<div style='font-family:JetBrains Mono,monospace;font-size:11px;color:#00d4ff;margin-top:4px;'>"
            + cid + " · " + pkg + " · " + status_label + "</div></div>"
        ...
        st.markdown(
            "<div style='background:linear-gradient(135deg,#ffd70011,#ffab0011);"
            "border:1px solid #ffd70033;border-left:3px solid #ffd700;"
            ...
            "color:#ffd700;margin-bottom:4px;'>🎉 Dapatkan Komisi Penjualan 10%!</div>"
            "<div style='font-size:13px;color:#9ab8d4;'>Referensikan rekan bisnis Anda menggunakan link referral unik Anda "
            "dan dapatkan <strong style='color:#ffd700;'>komisi 10% dari biaya setup</strong> "
            "untuk setiap klien baru yang berhasil bergabung.</div></div>",
            unsafe_allow_html=True,
        )
        ...
        with portal_tabs[0]:
            c1, c2 = st.columns(2)
            with c1:
                status_color = "#00e676" if status == "Aktif" else "#ffab00"
                st.markdown(
                    ...
                    "<div style='font-size:13px;color:#7a9bbf;margin-bottom:8px;'><b style='color:#e8f4ff;'>Status:</b> "
                    "<span style='color:" + status_color + ";'>" + status + "</span></div>"
                    "<div style='font-size:13px;color:#7a9bbf;'><b style='color:#e8f4ff;'>Client ID:</b> "
                    "<span style='font-family:JetBrains Mono,monospace;color:#00d4ff;'>" + cid + "</span></div></div>",
                    unsafe_allow_html=True,
                )
            with c2:
                if status == "Aktif":
                    st.markdown(
                        "<div style='background:#00e67611;border:1px solid #00e67633;border-radius:12px;padding:20px;'>"
                        "<div style='font-size:12px;color:#00e676;font-family:JetBrains Mono,monospace;"
                        "text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;'>✅ Akun Aktif</div>"
                        "<div style='font-size:13px;color:#7a9bbf;'>Sistem V-Guard AI aktif mengawasi bisnis Anda 24/7.</div>"
                        "<div style='margin-top:12px;font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;color:#00e676;'>Live</div></div>",
                        unsafe_allow_html=True,
                    )
                else:
                    ...
                    st.markdown(
                        ...
                        "<div style='font-size:12px;color:#ffab00;font-family:JetBrains Mono,monospace;"
                        "text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;'>⏳ Menunggu Aktivasi</div>"
                        "<div style='font-size:13px;color:#7a9bbf;margin-bottom:12px;'>Selesaikan pembayaran untuk mengaktifkan akun Anda.</div></div>",
                        unsafe_allow_html=True,
                    )
        ...
        with portal_tabs[1]:
            ...
            st.markdown(
                ...
                "<div style='font-size:13px;color:#7a9bbf;margin-bottom:16px;'>"
                "Bagikan link ini ke rekan bisnis Anda. Setiap referral yang berhasil aktivasi, Anda mendapatkan "
                "<strong style='color:#ffd700;'>komisi 10% dari biaya setup</strong>!</div>"
                "<div class='ref-link-box'>" + ref_link + "</div>"
                "<div style='margin-top:12px;font-size:12px;color:#7a9bbf;'>"
                "💰 <b>Contoh komisi:</b><br>"
                "V-PRO (setup Rp 750.000) → Komisi <b style='color:#ffd700;'>Rp 75.000</b><br>"
            ...
            wa_share_txt = urllib.parse.quote(
                "Halo! Saya sudah pakai V-Guard AI untuk keamanan bisnis saya — hasilnya luar biasa!\n\n"
                "Sistem AI ini otomatis deteksi fraud kasir, pantau stok, & audit rekening bank 24/7.\n\n"
                "Coba daftar di sini (link khusus saya): " + ref_link
            )
            ...
            if my_refs:
                st.markdown(
                    "<div style='margin-top:20px;font-family:Rajdhani,sans-serif;font-size:16px;"
                    "font-weight:700;color:#e8f4ff;margin-bottom:12px;'>📊 Referral Saya</div>",
                    unsafe_allow_html=True,
                )
                ...
                for r in my_refs:
                    ...
                    st.markdown(
                        ...
                        "<div>"
                        "<div style='font-size:14px;color:#e8f4ff;font-weight:500;'>"
                        + r.get("new_client","—") + " — " + r.get("new_usaha","—") + "</div>"
                        "<div style='font-size:12px;color:#7a9bbf;'>"
                        + r.get("paket","—") + " · Setup: Rp {:,.0f} · Komisi: <b style='color:#ffd700;'>Rp {:,.0f}</b></div>".format(r.get("setup_fee",0), r.get("komisi_10pct",0)) +
        ...
        with portal_tabs[2]:
            if status == "Aktif":
                st.markdown(
                    "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
                    "color:#e8f4ff;margin-bottom:16px;'>📊 Dashboard Ringkasan</div>",
                    unsafe_allow_html=True,
                )
                ...
                if pkg in ("V-ADVANCE","V-ELITE","V-ULTRA"):
                    ...
                    st.dataframe(
                        df_sample[["ID_Transaksi","Kasir","Jumlah","Status","Waktu"]].head(6),
        ...
        with portal_tabs[3]:
            st.markdown(
                "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
                "color:#e8f4ff;margin-bottom:16px;'>📁 Folder Dokumen</div>",
                unsafe_allow_html=True,
            )
            docs = [
                ("📋","Kontrak Langganan",   "Dokumen perjanjian layanan V-Guard AI",             "Tersedia"),
                ("🧾","Invoice Terakhir",    "Invoice " + datetime.date.today().strftime("%B %Y"), "PDF"),
                ("📘","Panduan Onboarding",  "Langkah aktivasi & setup sistem",                   "Tersedia"),
                ("🔒","Kebijakan Privasi",   "SLA & data privacy agreement",                      "Tersedia"),
                ("📊","Laporan Bulanan",     "Laporan AI " + datetime.date.today().strftime("%B %Y"),
                 "PDF" if status == "Aktif" else "Dalam Proses"),
            ]
            for icon, name, desc, badge in docs:
                ...
                st.markdown(
                    ...
                    "<span style='font-size:22px;'>" + icon + "</span>"
                    "<div style='flex:1;'>"
                    "<div style='font-size:14px;color:#e8f4ff;font-weight:500;'>" + name + "</div>"
                    "<div style='font-size:12px;color:#7a9bbf;'>" + desc + "</div></div>"
                    "<span style='font-family:JetBrains Mono,monospace;font-size:10px;color:" + badge_color + ";"
                    "border:1px solid " + badge_color + "44;background:" + badge_color + "11;"
                    "padding:3px 10px;border-radius:20px;'>" + badge + "</span></div>",
                    unsafe_allow_html=True,
                )
        ...
        with tab_login:
            ...
            with lc:
                ...
                st.markdown(
                    ...
                    "<div style='font-family:Rajdhani,sans-serif;font-size:20px;font-weight:700;color:#e8f4ff;'>Login Portal Klien</div>"
                    "<div style='font-size:13px;color:#7a9bbf;margin-top:4px;'>Masukkan Client ID yang diberikan tim V-Guard</div></div>",
                    unsafe_allow_html=True,
                )
        ...
        with tab_daftar:
            if st.session_state.get("portal_form_submitted"):
                st.success("✅ Pengajuan Anda telah diterima! Tim V-Guard akan menghubungi Anda dalam 1×24 jam.")
                ...
                with fc:
                    ...
                    st.markdown(
                        ...
                        "color:#00d4ff;margin-bottom:16px;'>📝 Formulir Pendaftaran</div>",
                        unsafe_allow_html=True,
                    )
                    ...
                    paket_form = st.selectbox("Paket yang Diminati *",
                                              ["V-LITE","V-PRO","V-ADVANCE","V-ELITE","V-ULTRA"],      key="form_paket")
                    catatan    = st.text_area("Ceritakan singkat bisnis Anda",
                                              placeholder="Toko kelontong 2 kasir, omzet ±20jt/bln...",
                                              height=80, key="form_catatan")
                    ...
                    if ref_src:
                        st.markdown(
                            ...
                            "🔗 Anda mendaftar melalui link referral: <b>" + ref_src + "</b></div>",
                            unsafe_allow_html=True,
                        )
                    ...
                    if st.button("Kirim Pengajuan", type="primary", use_container_width=True, key="btn_submit_form"):
                        if nama_klien and nama_usaha and wa_klien:
                            ...
                            if ref_src:
                                setup_fee_map = {
                                    "V-LITE": 250_000, "V-PRO": 750_000,
                                    "V-ADVANCE": 3_500_000, "V-ELITE": 10_000_000, "V-ULTRA": 0,
                                }
                                setup_fee = setup_fee_map.get(paket_form, 0)
                                if setup_fee > 0:
...
# =============================================================================
# 19. PAGE: INVESTOR AREA (Protected)
# =============================================================================
    ...
    st.markdown(
        "<div style='background:linear-gradient(135deg,#0d1626,#1a1500);border:1px solid #ffd70033;"
        "border-radius:14px;padding:24px;margin-bottom:24px;'>"
        "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#ffd700;"
        "letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;'>CONFIDENTIAL — INVESTOR BRIEF</div>"
        "<div style='font-family:Rajdhani,sans-serif;font-size:28px;font-weight:700;color:#ffd700;margin-bottom:4px;'>"
        "V-Guard AI Intelligence</div>"
        "<div style='font-size:14px;color:#7a9bbf;'>AI-Powered Business Security Ecosystem · Indonesia Market</div></div>",
        unsafe_allow_html=True,
    )
    ...
    for i, (title, val, desc) in enumerate(inv_stats):
        with cols[i % 3]:
            st.markdown(
                ...
                "<div class='investor-stat-num'>" + val + "</div>"
                "<div style='font-family:Rajdhani,sans-serif;font-size:14px;font-weight:700;"
                "color:#e8f4ff;margin-top:4px;'>" + title + "</div>"
                "<div class='investor-stat-lbl'>" + desc + "</div></div>",
                unsafe_allow_html=True,
            )
    ...
    with inv_tabs[0]:
        ...
        st.markdown(
            "<div style='background:#101c2e;border:1px solid #1e3352;border-radius:8px;"
            "padding:14px 18px;margin-top:12px;font-size:12px;color:#7a9bbf;'>"
            "⚠️ Proyeksi berdasarkan asumsi pertumbuhan organik + digital marketing. "
            "Angka aktual dapat berbeda tergantung eksekusi dan kondisi pasar.</div>",
            unsafe_allow_html=True,
        )
    ...
    with inv_tabs[1]:
        ...
        for quarter, title, desc, status_r in roadmap:
            ...
            st.markdown(
                "<div style='background:#101c2e;border:1px solid #1e3352;border-left:3px solid " + color + ";"
                "border-radius:8px;padding:16px 20px;margin-bottom:10px;display:flex;"
                "align-items:flex-start;gap:16px;'>"
                "<div style='font-family:JetBrains Mono,monospace;font-size:11px;color:" + color + ";"
                "min-width:60px;margin-top:2px;'>" + quarter + "</div>"
                "<div>"
                "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#e8f4ff;'>" + title + "</div>"
                "<div style='font-size:13px;color:#7a9bbf;margin-top:4px;'>" + desc + "</div>"
                "</div></div>",
                unsafe_allow_html=True,
            )
    ...
    with inv_tabs[2]:
        st.markdown(
            ...
            "● Langganan bulanan (SaaS MRR)<br>● Biaya setup & implementasi<br>"
            "● White-label licensing (V-ULTRA)<br>● Komisi referral program<br>● API usage fees (enterprise)</div></div>"
            "<div style='background:#101c2e;border:1px solid #1e3352;border-radius:12px;padding:20px;'>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:16px;font-weight:700;color:#ffd700;margin-bottom:12px;'>"
            "🛡️ Keunggulan Kompetitif</div>"
            "<div style='font-size:13px;color:#7a9bbf;line-height:1.8;'>"
            "● AI local-first (bahasa & konteks Indonesia)<br>● 5-tier pricing menjangkau semua segmen<br>"
            "● CCTV + Kasir + Bank dalam 1 platform<br>● Plug & Play — tidak butuh IT team<br>"
            "● WhatsApp-native alert system</div></div></div>",
            unsafe_allow_html=True,
        )
...
# =============================================================================
# 20. PAGE: ADMIN ACCESS — WAR ROOM (Protected)
# =============================================================================
    ...
    with war_tabs[0]:
        ...
        <div class='demo-mockup'>
            ...
            <span style='color:#00e676;'>✓</span> [SYSTEM] V-Guard AI v3.0 — Boot selesai<br>
        ...
        if dp_log:
            ...
            st.markdown(
                "<div class='match-banner' style='margin-top:16px;'>"
                "<div class='match-banner-title'>🎯 Paket Terdeteksi dari Sesi Chat CS</div>"
                "<div class='match-banner-body'>Cocok: <b>" + dp_log + "</b> — " + bul + "/bln</div></div>",
                unsafe_allow_html=True,
            )
    ...
    with war_tabs[1]:
        st.markdown(
            ...
            "color:#e8f4ff;margin-bottom:4px;'>🤖 10 Elite AI Agent Squad</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:20px;'>"
            "Status real-time seluruh agen AI dalam ekosistem V-Guard.</div>",
            unsafe_allow_html=True,
        )
        ...
        for row_start in range(0, 10, 2):
            ...
            for i, col in enumerate(cols):
                ...
                color = color_map.get(st_code, "#7a9bbf")
                label = label_map.get(st_code, "UNKNOWN")
                ks    = st.session_state.agent_kill_switch.get(agent["id"], False)
                with col:
                    card_border  = "agent-card-active" if not ks else "agent-card-offline"
                    status_color = "#ff3b5c" if ks else color
                    status_lbl   = "KILLED"  if ks else label
                    st.markdown(
                        "<div class='agent-card " + card_border + "'>"
                        "<div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;'>"
                        "<div style='display:flex;align-items:center;gap:8px;'>"
                        "<span style='font-size:22px;'>" + agent["icon"] + "</span>"
                        "<div>"
                        "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#e8f4ff;'>"
                        "Agent #" + str(agent["id"]) + " · " + agent["name"] + "</div>"
                        "<div style='font-size:11px;color:#7a9bbf;'>" + agent["role"] + "</div>"
                        "</div></div>"
                        "<span style='font-family:JetBrains Mono,monospace;font-size:9px;"
                        "color:" + status_color + ";border:1px solid " + status_color + "44;"
                        "background:" + status_color + "11;padding:2px 8px;border-radius:20px;'>"
                        + status_lbl + "</span></div>"
                        "<div style='font-size:12px;color:#7a9bbf;line-height:1.5;'>" + agent["desc"] + "</div></div>",
                        unsafe_allow_html=True,
                    )
    ...
    with war_tabs[2]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>📹 Monitor CCTV — Visual Cashier Audit</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:20px;'>"
            "Live feed dari kamera klien. Teks transaksi dirender otomatis oleh VisualEye AI.</div>",
            unsafe_allow_html=True,
        )
        cam_locations = [
            "Outlet Sudirman — Kasir 1",
            "Outlet Sudirman — Kasir 2",
            "Resto Central — Kasir Utama",
            "Cabang Tangerang — Pintu Masuk",
        ]
        ...
        for i, loc in enumerate(cam_locations):
            with cam_cols[i % 2]:
                ...
                alert_show = (i == 0)
                alert_html = (
                    "<div class='cctv-alert'>⚠ VOID DETECTED</div>"
                    if alert_show else
                    "<div style='position:absolute;top:8px;right:8px;font-family:JetBrains Mono,monospace;"
                    "font-size:10px;color:#00e676;background:#00000088;padding:4px 8px;border-radius:4px;'>✓ NORMAL</div>"
                )
                st.markdown(
                    ...
                    "</div>"
                    "<div class='cctv-overlay'>● REC " + now_str + " · Cam " + str(i+1) + "</div>"
                    + alert_html + "</div>",
                    unsafe_allow_html=True,
                )
        ...
        st.markdown(
            ...
            "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#00d4ff;margin-bottom:10px;'>"
            "VisualEye AI — YOLO Integration</div>"
            ...
            "Integrasi: <code style='color:#00d4ff;'>process_yolo_cctv_frame()</code> tersedia di kode — "
            "hubungkan DVR CCTV via HTTP POST.</div></div>",
            unsafe_allow_html=True,
        )
    ...
    with war_tabs[3]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>🚨 Fraud Intelligence Scanner — The Liaison</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:16px;'>"
            "Agent #4 Liaison: Filter lokal anomali sebelum dikirim ke API — hemat biaya hingga 80%.</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "<div style='background:#ff3b5c11;border:1px solid #ff3b5c33;border-left:3px solid #ff3b5c;"
            "border-radius:8px;padding:12px 16px;margin-bottom:16px;'>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:14px;font-weight:700;color:#ff3b5c;margin-bottom:4px;'>"
            "🤖 AI Fraud Alarm Engine</div>"
            "<div style='font-size:12px;color:#7a9bbf;'>"
            "Komparasi data kasir POS dengan deteksi YOLO CCTV. Mismatch = alarm otomatis ke Owner.</div></div>",
            unsafe_allow_html=True,
        )

        if st.button("⚡ Jalankan AI Fraud Alarm Check", key="btn_ai_alarm"):
            yolo_results = [
                process_yolo_cctv_frame({"camera_id": "cam_0" + str(i+1), "timestamp": str(datetime.datetime.now())})
                for i in range(2)
            ]
            ...
            if alarms:
                ...
                for alarm in alarms:
                    ...
                    st.markdown(
                        "<div style='background:#ff3b5c11;border:1px solid #ff3b5c33;"
                        "border-radius:8px;padding:12px 16px;margin-bottom:8px;'>"
                        "<b style='color:#ff3b5c;'>⚠ ALARM</b><br>"
                        "Kasir: " + alarm["kasir"] + " · Cabang: " + alarm["cabang"] +
                        " · Jumlah: Rp {:,.0f}".format(alarm["jumlah"]) + "<br>"
                        "Alasan: " + alarm["reason"] + "</div>",
                        unsafe_allow_html=True,
                    )
                    st.link_button(
                        "📲 Kirim Alert ke Owner — " + alarm["cabang"], wa_alarm,
                        key="alarm_" + alarm["kasir"] + "_" + alarm["cabang"],
                    )
            else:
        ...
        with tv:
            if not hasil["void"].empty:
                ...
                for cab in hasil["void"]["Cabang"].unique():
                    ...
                    st.link_button("Alert Owner — " + cab, alarm_link, key="voidalert_" + cab)
            else:
        ...
        if model_vguard:
            ...
            if st.button("🤖 Jalankan AI Deep Scan (Anomali Only)", type="primary", key="btn_deep_scan"):
                fraud_rows = pd.concat(
                    [hasil["void"], hasil["fraud"], hasil["suspicious"]]
                ).drop_duplicates(subset=["ID_Transaksi"])
                if fraud_rows.empty:
                    ...
                    with st.spinner("Agent Liaison + Gemini menganalisis anomali..."):
                        try:
                            prompt = (
                                "Analisis HANYA data anomali ini (transaksi normal sudah difilter Agent Liaison):\n\n"
                                + fraud_rows.to_string(index=False)
                                + "\n\nBerikan: 1) Pola kecurangan yang terdeteksi, "
                                "2) Kasir paling berisiko, "
                                "3) Rekomendasi taktis untuk Owner. Format ringkas dan actionable."
                            )
    ...
    with war_tabs[4]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>📋 Pengajuan Masuk — Antrian Aktivasi</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:20px;'>"
            "Pengajuan dari Portal Klien yang menunggu proses aktivasi.</div>",
            unsafe_allow_html=True,
        )
        ...
        if not pending_list:
            ...
            for i, klien in enumerate(pending_list):
                ...
                if klien.get("Ref"):
                    ref_badge = (
                        "<span style='background:#ffd70011;color:#ffd700;border:1px solid #ffd70044;"
                ...
                st.markdown(
                    ...
                    "<div>"
                    "<div style='font-family:Rajdhani,sans-serif;font-size:17px;font-weight:700;color:#e8f4ff;'>"
                    + klien.get("Nama Klien","—") + " — " + klien.get("Nama Usaha","—") + "</div>"
                    "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#00d4ff;margin-bottom:4px;'>"
                    + cid + " · " + pkg + " · " + hb + "/bln " + ref_badge + "</div>"
                    "<div style='font-size:12px;color:#7a9bbf;'>WA: " + klien.get("WhatsApp","—") +
                    " · Daftar: " + klien.get("Tanggal","—") + "</div>"
                    + ("<div style='font-size:12px;color:#7a9bbf;margin-top:4px;'>Catatan: " + klien.get("Catatan","") + "</div>" if klien.get("Catatan") else "")
                    + "</div><span class='client-badge-pending'>Menunggu Pembayaran</span></div></div>",
                    unsafe_allow_html=True,
                )
                ...
                with pc1:
                    pay_link = buat_payment_link(pkg, klien.get("Nama Klien","Klien"))
                    st.link_button("💳 Kirim Link Pembayaran", pay_link,
                                   use_container_width=True, key="pay_" + cid + "_" + str(i))
                with pc2:
                    db_idx = next(
                        (j for j, k in enumerate(st.session_state.db_umum) if k.get("Client_ID") == cid), None
                    )
                    if db_idx is not None:
                        if st.button("✅ Aktivasi Akun", key="aktiv_" + cid + "_" + str(i),
                                     type="primary", use_container_width=True):
                            st.session_state.db_umum[db_idx]["Status"] = "Aktif"
                            for r in st.session_state.db_referrals:
                                if (r.get("new_client") == klien.get("Nama Klien") and
                                        r.get("status") == "Menunggu Konfirmasi"):
                                    r["status"] = "Terbayar"
                            st.rerun()
                with pc3:
                    akses_txt = urllib.parse.quote(
                        "Halo " + klien.get("Nama Klien","Klien") + ",\n\n"
                        "✅ Akun V-Guard Anda telah AKTIF!\n\n"
                        "🔑 Client ID: " + cid + "\n"
                        "🔗 Dashboard: " + BASE_APP_URL + "\n"
                        "📦 Paket: " + pkg + "\n\n"
                        "Selamat menggunakan V-Guard AI!\n— Tim V-Guard AI"
                    )
                    st.link_button("📲 Kirim Akses Klien",
                                   "https://wa.me/" + wa_tgt + "?text=" + akses_txt,
                                   use_container_width=True, key="access_" + cid + "_" + str(i))
    ...
    with war_tabs[5]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>👥 Manajemen Klien Aktif</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:16px;'>"
            "Kelola status, kirim invoice, dan akses dashboard klien.</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander("➕ Tambah Klien Manual"):
            ...
            with ac2:
                ...
                new_due    = st.date_input(
                    "Jatuh Tempo Invoice (H-7 reminder)",
                    key="new_due",
        ...
        if not st.session_state.db_umum:
            ...
            for i, klien in enumerate(st.session_state.db_umum):
                ...
                hb, hs   = HARGA_MAP.get(klien.get("Produk","V-LITE"), ("—","—"))
                ...
                st.markdown(
                    ...
                    "<div style='font-family:Rajdhani,sans-serif;font-size:17px;font-weight:700;color:#e8f4ff;'>"
                    + klien.get("Nama Klien","—") + " — " + klien.get("Nama Usaha","-") + "</div>"
                    "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#00d4ff;margin-bottom:6px;'>"
                    + cid + " · " + klien.get("Produk","V-LITE") + "</div>"
                    "<div style='font-size:12px;color:#7a9bbf;margin-bottom:8px;'>"
                    "WA: " + klien.get("WhatsApp","-") + " · " + hb + "/bln · Setup: " + hs + "</div>"
                    "<span class='" + badge_cls + "'>" + status_txt + "</span></div>",
                    unsafe_allow_html=True,
                )
                ...
                with ac2:
                    inv_txt = urllib.parse.quote(
                        "INVOICE V-GUARD AI\n\nYth. " + klien.get("Nama Klien","") + "\n"
                        "Paket: " + klien.get("Produk","") + "\n"
                        "Bulanan: " + hb + "\nSetup: " + hs + "\n\n"
                        "Transfer: BCA 3450074658 a/n Erwin Sinaga\nKonfirmasi setelah transfer.\n— Tim V-Guard AI"
                    )
                    st.link_button("🧾 Invoice",
                                   "https://wa.me/" + wa_tgt + "?text=" + inv_txt,
                                   use_container_width=True, key="inv_" + str(i))
                with ac3:
                    akses_txt = urllib.parse.quote(
                        "Halo " + klien.get("Nama Klien","") + ",\n\n✅ Akun V-Guard Anda AKTIF!\n\n"
                        "🔑 Client ID: " + cid + "\n🔗 Portal: " + BASE_APP_URL + "\n"
                        "📦 Paket: " + klien.get("Produk","") + "\n\n— Tim V-Guard AI"
                    )
                    st.link_button("🔑 Kirim Akses",
                                   "https://wa.me/" + wa_tgt + "?text=" + akses_txt,
                                   use_container_width=True, key="akses_" + str(i))
                with ac4:
                    ref_link2 = buat_referral_link(cid)
                    ref_txt   = urllib.parse.quote(
                        "Link referral Anda: " + ref_link2 + "\n"
                        "Komisi 10% dari setiap klien baru yang berhasil!"
                    )
                    st.link_button("🔗 Referral",
                                   "https://wa.me/" + wa_tgt + "?text=" + ref_txt,
                                   use_container_width=True, key="ref_" + str(i))
                st.markdown("<div style='height:4px;'></div>", unsafe_allow_html=True)
    ...
    with war_tabs[6]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>🔗 Referral Dashboard</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:20px;'>"
            "Monitor daftar referral masuk, status penjualan, dan kalkulasi komisi 10%.</div>",
            unsafe_allow_html=True,
        )
        ...
        rm2.metric("Total Komisi",     "Rp {:,.0f}".format(total_kom_r))
        rm3.metric("Komisi Pending",   "Rp {:,.0f}".format(kom_pending))
        rm4.metric("Komisi Terbayar",  "Rp {:,.0f}".format(kom_terbayar))

        st.markdown("<div style='height:16px;'></div>", unsafe_allow_html=True)

        with st.expander("➕ Input Referral Manual (testing)"):
            ...
            with mr2:
                ...
                st.markdown(
                    "<div style='padding:12px;background:#ffd70011;border:1px solid #ffd70033;"
                    "border-radius:8px;margin-top:24px;font-size:13px;color:#ffd700;'>"
                    "Setup Fee: Rp {:,.0f}<br>Komisi 10%: <b>Rp {:,.0f}</b></div>".format(demo_setup, demo_komisi),
                    unsafe_allow_html=True,
                )
            if st.button("Tambah Referral", key="btn_add_ref", type="primary"):
                if demo_ref_cid and demo_new_nama:
                    track_referral(
                        demo_ref_cid,
                        {"Nama Klien": demo_new_nama, "Nama Usaha": demo_new_usaha, "Produk": demo_paket},
                        demo_setup,
                    )
        ...
        if not db_refs:
            ...
            st.markdown(
                "<div style='margin-bottom:12px;font-family:Rajdhani,sans-serif;font-size:16px;"
                "font-weight:700;color:#e8f4ff;'>Daftar Referral Masuk</div>",
                unsafe_allow_html=True,
            )
            for i, r in enumerate(db_refs):
                ...
                st.markdown(
                    ...
                    "<div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;'>"
                    "<div>"
                    "<div style='font-size:14px;color:#e8f4ff;font-weight:600;'>"
                    + r.get("new_client","—") + " — " + r.get("new_usaha","—") + "</div>"
                    "<div style='font-size:12px;color:#7a9bbf;margin-top:2px;'>"
                    "Dari Ref: <b style='color:#00d4ff;font-family:JetBrains Mono,monospace;'>"
                    + r.get("ref_cid","—") + "</b> · "
                    "Paket: " + r.get("paket","—") + " · "
                    "Setup: Rp {:,.0f} · Komisi: <b style='color:#ffd700;'>Rp {:,.0f}</b></div>".format(
                        r.get("setup_fee",0), r.get("komisi_10pct",0)
                    ) +
                ...
                with rb1:
                    if not is_paid:
                        if st.button("✅ Tandai Komisi Terbayar", key="pay_ref_" + str(i), use_container_width=True):
                ...
                with rb2:
                    referrer = next(
                        (k for k in st.session_state.db_umum if k.get("Client_ID","").upper() == r.get("ref_cid","").upper()), None
                    )
                    if referrer and referrer.get("WhatsApp"):
                        wa_ref = referrer["WhatsApp"]
                        if not wa_ref.startswith("62"):
                            wa_ref = "62" + wa_ref.lstrip("0")
                        komisi_msg = urllib.parse.quote(
                            "Halo " + referrer.get("Nama Klien","") + ",\n\n"
                            "🎉 Selamat! Referral Anda berhasil!\n\n"
                            "Klien: " + r.get("new_client","—") + "\n"
                            "Paket: " + r.get("paket","—") + "\n"
                            "Komisi 10%: Rp {:,.0f}\n\n".format(r.get("komisi_10pct",0)) +
                            "Komisi akan ditransfer ke rekening Anda segera.\n— Tim V-Guard AI"
                        )
                        st.link_button("📲 Notif Komisi",
                                       "https://wa.me/" + wa_ref + "?text=" + komisi_msg,
                                       use_container_width=True, key="notif_ref_" + str(i))
                st.markdown("<div style='height:4px;'></div>", unsafe_allow_html=True)
            ...
            st.download_button(
                ...
                file_name="vguard_referrals_" + str(datetime.date.today()) + ".csv",
                mime="text/csv",
            )
    ...
    with war_tabs[7]:
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;"
            "color:#e8f4ff;margin-bottom:4px;'>⏰ Auto-Billing — H-7 & H-1 Reminder</div>"
            "<div style='font-size:13px;color:#7a9bbf;margin-bottom:20px;'>"
            "Agent Collector #9 mengingatkan klien otomatis H-7 dan H-1 sebelum jatuh tempo invoice.</div>",
            unsafe_allow_html=True,
        )
        ...
        if reminders:
            ...
            for rem in reminders:
                ...
                msg_rem  = urllib.parse.quote(
                    ("🚨 URGENT — " if rem["delta"] == 1 else "⏰ ") +
                    "PENGINGAT INVOICE V-GUARD AI\n\nYth. " + rem["nama"] + ",\n\n"
                    "Invoice paket " + rem["paket"] + " Anda akan jatuh tempo pada " + rem["due"] +
                    " (" + due_info + ").\n\n"
                    "Mohon segera lakukan pembayaran agar layanan tidak terputus.\n\n"
                    "Transfer: BCA 3450074658 a/n Erwin Sinaga\n— Tim V-Guard AI"
                )
                st.markdown(
                    "<div style='background:#101c2e;border:1px solid " + color + "44;"
                    ...
                    "<div style='font-size:12px;color:#7a9bbf;'>"
                    "Paket: " + rem["paket"] + " · Due: " + rem["due"] + " · WA: " + rem["wa"] + "</div></div>",
                    unsafe_allow_html=True,
                )
                if wa_rem:
                    st.link_button(
                        "📲 Kirim Reminder ke " + rem["nama"],
                        "https://wa.me/" + wa_rem + "?text=" + msg_rem,
                        key="rem_" + rem["cid"],
                    )
        else:
        ...
        st.markdown(
            "<div style='background:#101c2e;border:1px solid #1e3352;border-radius:10px;padding:16px 20px;'>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#00d4ff;margin-bottom:10px;'>"
            "⚙️ Setup Auto-Billing Otomatis</div>"
            "<div style='font-size:12px;color:#7a9bbf;line-height:1.8;'>"
            ...
            "2. Set <code style='color:#00d4ff;'>invoice_due_date</code> saat klien aktif<br>"
            "3. Jadwalkan <code style='color:#00d4ff;'>check_autobilling_reminders()</code> via cron job setiap 08:00 WIB<br>"
            "4. Hubungkan response ke endpoint WA gateway untuk kirim otomatis</div></div>",
            unsafe_allow_html=True,
        )
    ...
    with war_tabs[8]:
        if st.session_state.db_umum:
            ...
            st.download_button(
                ...
                file_name="vguard_clients_" + str(datetime.date.today()) + ".csv",
                mime="text/csv",
            )
            st.markdown("<div style='height:8px;'></div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    "</div>"
    "<div style='font-size:12px;color:#7a9bbf;'>"
    "Digitizing Trust · Eliminating Leakage · Protecting Every Rupiah</div></div>",
    unsafe_allow_html=True,
)
