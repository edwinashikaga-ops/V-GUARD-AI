...
def menu_visionary():
    """Agent #1 — Mata: CCTV & Visual Cashier Audit Log"""
    squad     = get_squad()
    visionary = squad.agent(1)

    st.markdown("""
    <div style='padding:32px 48px 0;'>
        <div class='page-title'>👁️ Visionary — <span style='color:#00d4ff;'>CCTV Visual Audit</span></div>
        <div class='page-subtitle'>Agent #1 · Mata Sistem · Real-time CCTV overlay & anomali visual kasir.</div>
    </div>""", unsafe_allow_html=True)

    is_active    = visionary.is_active()
    status_color = "#00e676" if is_active else "#ff3b5c"
    status_label = "ACTIVE — Mengawasi feed CCTV" if is_active else "KILLED — Tidak aktif"

    st.markdown(
        f"<div style='background:#101c2e;border:1px solid #1e3352;border-left:3px solid {status_color};"
        f"border-radius:10px;padding:14px 20px;margin:16px 48px 0;display:flex;"
        ...
        f"Model: YOLOv8-Custom · Labels: unscanned_item, void_gesture, cash_swap, item_concealment</div></div>"
        f"<div style='font-family:Rajdhani,sans-serif;font-size:13px;font-weight:700;color:#7a9bbf;'>"
        f"Log entries: {len(visionary._log)}</div></div>",
    ...
    with col_run:
        run_scan  = st.button("▶ Jalankan Batch Scan (Demo)", type="primary",
                              key="vis_run", use_container_width=True)
    with col_reset:
        clear_log = st.button("🗑 Bersihkan Log", key="vis_clear", use_container_width=True)
    with col_filt:
    ...
    if run_scan:
        ...
        if alert_count:
            st.error(f"⚠️ {alert_count} frame mengandung ALERT! Cek tabel di bawah.")
        else:
            st.success(f"✅ {len(results)} frame diproses — tidak ada anomali visual.")
    ...
    alert_rate   = f"{total_alerts/total_frames*100:.1f}%" if total_frames > 0 else "—"

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Total Frame Diproses", str(total_frames))
    m2.metric("Alert Terdeteksi",     str(total_alerts),
    ...
    st.markdown(
        ...
        "color:#e8f4ff;margin-bottom:10px;'>📋 Log Frame CCTV</div>",
        unsafe_allow_html=True,
    )
    ...
    if not vision_logs:
        ...
        display_logs = [v for v in vision_logs if v.get("alert")] if filter_alert else vision_logs
        if not display_logs:
            ...
            for v in reversed(display_logs[-50:]):
                ...
                rows.append({
                    ...
                    "Overlay":   (overlay_txt[:60] + "…") if len(overlay_txt) > 60 else overlay_txt,
                })
...
**Production:** Ganti `process_yolo_cctv_frame()` stub dengan inferensi `ultralytics.YOLO` asli.
...
def menu_treasurer():
    ...
    st.markdown("""
    <div style='padding:32px 48px 0;'>
        <div class='page-title'>💰 Treasurer — <span style='color:#00d4ff;'>Profit & Expense</span></div>
        <div class='page-subtitle'>Agent #10 · Kantong Sistem · MRR, margin, proyeksi, dan tagihan WhatsApp otomatis.</div>
    </div>""", unsafe_allow_html=True)
    ...
    with col_calc:
        do_calc = st.button("🔄 Hitung Ledger Sekarang", type="primary",
                            key="tr_calc", use_container_width=True)
    with col_months:
        months_proj = st.number_input("Proyeksi (bulan)", min_value=1, max_value=24,
    ...
    with st.expander("➕ Tambah Pengeluaran Ekstra Bulan Ini"):
        ...
        with ec1:
            ...
            extra_val1   = st.number_input("Nominal (Rp)", min_value=0, value=500_000,
                                           step=100_000, key="ex_val1")
        with ec2:
            extra_label2 = st.text_input("Label 2", value="Domain & SSL",     key="ex_lbl2")
            extra_val2   = st.number_input("Nominal (Rp)", min_value=0, value=150_000,
                                           step=50_000, key="ex_val2")
        with ec3:
            extra_label3 = st.text_input("Label 3", value="Operasional Lain", key="ex_lbl3")
            extra_val3   = st.number_input("Nominal (Rp)", min_value=0, value=0,
    ...
    if do_calc or st.session_state.get("tr_last_ledger"):
        if do_calc:
            ...
            ledger_result = st.session_state.get("tr_last_ledger")
    else:
    ...
    st.markdown(
        "<div style='font-family:Rajdhani,sans-serif;font-size:17px;font-weight:700;"
        "color:#e8f4ff;margin:16px 0 12px;'>📊 Ringkasan Bulan Ini — "
        + ledger.get("period", datetime.date.today().strftime("%B %Y")) + "</div>",
        unsafe_allow_html=True,
    )
    ...
    k3.metric("Gross Margin", f"{ledger.get('gross_margin_pct',0):.1f}%", delta="Target 78%")
    ...
    with col_left:
        ...
        st.markdown(
            ...
            "color:#00d4ff;margin-bottom:14px;'>🧾 Breakdown Pengeluaran</div>",
            unsafe_allow_html=True,
        )
        all_costs = {**fixed_bkd, **{k: v for k, v in extra_expenses.items() if v > 0}}
        all_costs["COGS (22% Rev)"]  = int(ledger.get("cogs", 0))
        all_costs["Komisi Referral"] = int(ledger.get("komisi_expense", 0))
        ...
        for label, amount in all_costs.items():
            ...
            st.markdown(
                ...
                f"<span style='font-size:12px;color:#e8f4ff;font-weight:600;'>Rp {amount:,.0f}</span></div>"
                f"<div style='background:#1e3352;border-radius:4px;height:5px;'>"
                f"<div style='background:linear-gradient(90deg,#0091ff,#00d4ff);"
                f"width:{bar_pct}%;height:5px;border-radius:4px;'></div></div></div>",
                unsafe_allow_html=True,
            )
        st.markdown(
            f"<hr style='border-color:#1e3352;margin:12px 0;'>"
            f"<div style='display:flex;justify-content:space-between;'>"
            f"<span style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#e8f4ff;'>Total Biaya</span>"
            f"<span style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;color:#ff3b5c;'>"
        ...
        if pending_klien:
            st.markdown(
                "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;"
                "color:#ffab00;margin-bottom:10px;'>⏳ Tagih Klien Pending</div>",
                unsafe_allow_html=True,
            )
            for k in pending_klien:
                ...
                st.markdown(
                    f"<div style='background:#101c2e;border:1px solid #ffab0033;"
                    f"border-left:3px solid #ffab00;border-radius:8px;"
                    f"padding:12px 14px;margin-bottom:8px;'>"
                    f"<div style='font-size:13px;color:#e8f4ff;font-weight:600;'>{nama}</div>"
                    f"<div style='font-size:11px;color:#7a9bbf;'>{pkg_k} · {hb}/bln</div></div>",
                    unsafe_allow_html=True,
                )
                if wa:
                    st.link_button(
                        f"📲 Tagih {nama}",
                        f"https://wa.me/{wa}?text={inv_msg}",
                        key=f"tagih_{k.get('Client_ID','x')}",
    ...
    with col_right:
        if proj:
            ...
            st.markdown(
                "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;"
                "color:#00d4ff;margin-bottom:14px;'>📈 Proyeksi MRR & Net Profit</div>",
                unsafe_allow_html=True,
            )
            df_proj = pd.DataFrame(proj)
            st.dataframe(
                df_proj.rename(columns={
                    "month": "Bulan", "mrr": "MRR (Rp)",
                    "net_profit": "Net Profit (Rp)", "growth_pct": "Growth %"
                }),
            ...
            st.markdown(
                ...
                f"border-radius:8px;padding:10px 14px;margin-top:10px;font-size:13px;color:#00e676;'>"
                f"🏆 Proyeksi terbaik: <b>{best['month']}</b> — "
                f"Net Profit Rp {best['net_profit']:,.0f} (Growth {best['growth_pct']:.1f}%)</div>",
                unsafe_allow_html=True,
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.markdown(
            "<div style='font-family:Rajdhani,sans-serif;font-size:15px;font-weight:700;"
            "color:#ffd700;margin-bottom:14px;'>🧮 ROI per Klien Aktif</div>",
            unsafe_allow_html=True,
        )
        ...
        ledger_export = pd.DataFrame([ledger])
        st.download_button(
            "⬇️ Download Ledger CSV",
            data=ledger_export.to_csv(index=False).encode("utf-8"),
