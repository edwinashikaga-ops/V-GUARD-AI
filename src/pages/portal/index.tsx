...
import { startKasirSync, KasirTransaction } from "./VISIONARY_INTEGRATION";
...
export default function RegisterPage() {
  ...
  const [kasirTransactions, setKasirTransactions] = useState<KasirTransaction[]>([]);
  const [kasirLastSync, setKasirLastSync] = useState<number | null>(null);
  ...
  useEffect(() => {
    const stopSync = startKasirSync((transactions) => {
      setKasirTransactions(transactions);
      setKasirLastSync(Date.now());
    });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mb-6 rounded-3xl border border-slate-700 bg-slate-900/70 p-4 text-sm text-slate-300">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <span>Kasir sync aktif • {kasirTransactions.length} transaksi disimpan</span>
            <span>Last sync: {kasirLastSync ? new Date(kasirLastSync).toLocaleTimeString("id-ID") : "Menunggu sinkronisasi..."}</span>
          </div>
        </div>
